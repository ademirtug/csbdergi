...
- (UIImage *)cachedImage
{
	NSString* ab = [_URL absoluteString];
	NSArray* parts = [ab componentsSeparatedByString: @"/"];
	NSString* fnx = [parts objectAtIndex:[parts count]-1];
	
	
	UIImage* img = [_cache objectForKey:_URL];
	
	if( img == Nil )
	{
		if( is_cached(fnx) )
		{
			img = [UIImage imageWithContentsOfFile:[GetApplicationCacheDirectory() stringByAppendingPathComponent:fnx]];
			
			if( img != Nil )
			{
				[_cache setObject:img forKey:_URL];
			}
		}
	}
...
- (void)cacheImage:(UIImage *)image
{
	if (!_cancelled)
	{
        if (image && _URL)
        {
            ...
            if (storeInCache)
            {
				NSArray* parts = [[_URL absoluteString] componentsSeparatedByString: @"/"];
				NSString* fn = [GetApplicationCacheDirectory() stringByAppendingPathComponent:[parts objectAtIndex:[parts count]-1]];
				[UIImagePNGRepresentation(image) writeToFile:fn atomically:YES];
...
@implementation AsyncReflection
